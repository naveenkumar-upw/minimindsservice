...
// Fetch sections for the story
$story_sections = [];
$sections_result = $conn->query("SELECT * FROM story_sections WHERE story_id = $id ORDER BY sequence_number");
while ($section = $sections_result->fetch_assoc()) {
    $story_sections[] = $section;
}
        ...
        if (empty($errors)) {
            ...
            try {
                ...
                $updateFields = [
                    ...
                    "max_age=$max_age",
                    "moral='$moral'",
                    "moralExplanation='$moralExplanation'",
                ...
                if($conn->query("UPDATE stories SET " . implode(", ", $updateFields) . " WHERE id=$id")) {
                    // Delete existing sections
                    $conn->query("DELETE FROM story_sections WHERE story_id=$id");
                    ...
                    foreach($sequences as $key => $sequence) {
                        ...
                        $sectionQuery = "INSERT INTO story_sections (story_id, sequence_number, content, image) 
                                   VALUES ($id, $sequence_number, '$content', '$image')";
...
<head>
    ...
    <title>Edit Story - MiniMinds Service</title>
                ...
                <div class="form-section full-width">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="title" name="title" value="<?= htmlspecialchars($story['title']) ?>" required>
                        </div>
                        <div class="form-group">
                            ...
                            <select id="category" name="category_id" required>
                                ...
                                <?php foreach($categories as $cat): ?>
                                    <option value="<?= htmlspecialchars($cat['id']) ?>" 
                                            data-language="<?= htmlspecialchars($cat['language']) ?>"
                                            <?= $cat['id'] == $story['category_id'] ? 'selected' : '' ?>>
                        ...
                        <div class="form-group">
                            ...
                            <select id="language" name="language" required>
                                ...
                                ?>
                                    <option value="<?= $lang ?>" <?= $story['language'] === $lang ? 'selected' : '' ?>><?= $lang ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="read_time">Read Time (minutes)</label>
                            <input type="number" id="read_time" name="read_time" value="<?= htmlspecialchars($story['read_time']) ?>" min="1" required>
                        </div>
                        <div class="form-group">
                            <label for="country">Country</label>
                            <input type="text" id="country" name="country" value="<?= htmlspecialchars($story['country']) ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="state">State</label>
                            <input type="text" id="state" name="state" value="<?= htmlspecialchars($story['state']) ?>" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section full-width">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="number" id="min_age" name="min_age" value="<?= htmlspecialchars($story['min_age']) ?>" min="0" required>
                        </div>
                        <div class="form-group">
                            <label for="max_age">Maximum Age</label>
                            <input type="number" id="max_age" name="max_age" value="<?= htmlspecialchars($story['max_age']) ?>" min="0" required>
                        </div>
                        <div class="form-group" style="grid-column: 1 / -1;">
                            <label for="coverImage">Cover Image</label>
                            <input type="file" id="coverImage" name="coverImageUrl" accept="image/*">
                            <?php if ($story['coverImageUrl']): ?>
                                <div class="image-preview">
                                    <img src="../<?= htmlspecialchars($story['coverImageUrl']) ?>" alt="Current Cover Image" style="max-width:200px;">
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div class="form-section full-width">
                    ...
                    <div class="form-grid">
                        <div class="form-group" style="grid-column: 1 / -1;">
                            ...
                            <textarea id="moral" name="moral" required style="height: 100px;"><?= htmlspecialchars($story['moral']) ?></textarea>
                        </div>
                        <div class="form-group" style="grid-column: 1 / -1;">
                            <label for="moralExplanation">Moral Explanation</label>
                            <textarea id="moralExplanation" name="moralExplanation" required style="height: 150px;"><?= htmlspecialchars($story['moralExplanation']) ?></textarea>
                        </div>
                        <div class="form-group" style="grid-column: 1 / -1;">
                            ...
                            <?php 
                            $distractors = json_decode($story['distractors'] ?? '[]', true) ?: ['', '', '']; 
                            for($i = 1; $i <= 3; $i++): 
                            ?>
                                <div class="form-group">
                                    <label for="distractor<?= $i ?>">Distractor <?= $i ?></label>
                                    <input type="text" id="distractor<?= $i ?>" name="distractor<?= $i ?>" value="<?= htmlspecialchars($distractors[$i-1] ?? '') ?>" required>
                                </div>
                            <?php endfor; ?>
                        </div>
                    </div>
                        ...
                        <?php foreach ($story_sections as $section): ?>
                            <div class="story-section">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="sequence_number[]" value="<?= htmlspecialchars($section['sequence_number']) ?>" min="1" required>
                                </div>
                                <div class="form-group">
                                    <label>Section Content</label>
                                    <textarea name="section_content[]" required style="height: 120px;"><?= htmlspecialchars($section['content']) ?></textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <?php if ($section['image']): ?>
                                        <div class="image-preview">
                                            <img src="../<?= htmlspecialchars($section['image']) ?>" alt="Current Section Image" style="max-width:200px;">
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
